...
touch ejecutable.sh
echo -e " #Author: María Alejandra Moreno (cod_end = 3; cc_end =3)">> ejecutable.sh
echo -e "#Date 21-09-09 ">>ejecutable.sh
echo -e "#Version 1.0.0">> ejecutable.sh
...
echo -e "line=$(cat -v imensajeascci.txt | sed 's/\'^M'//gI')" >> ejecutable.sh  #---> utiliza sed para concatenar los espacio y sacar la informacion linea a linea 
echo -e "line_bi=$(cat -v imensajebin.txt | sed 's/\'^M'//gI')"  >> ejecutable.sh 
echo -e "line_oct=$(cat -v imensajeoct.txt | sed 's/\'^M'//gI')">> ejecutable.sh
...
echo -e "touch textobinario.txt textooctal.txt binariotexto.txt">>ejecutable.sh 

echo -e "textobinario '$line'>>textotextobinario.txt" >>ejecutable.sh 
echo -e "binariotexto '$line_bi'>>binariotexto.txt">>ejecutable.sh 
echo -e "textooctal '$line_oct' >>textooctal.txt">> ejecutable.sh 
